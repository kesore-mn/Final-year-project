...
# Settings table
query = "CREATE TABLE IF NOT EXISTS settings(id integer primary key, key VARCHAR(100), value VARCHAR(100))"
cursor.execute(query)

query = "CREATE TABLE IF NOT EXISTS downloads(id integer primary key, filename VARCHAR(255), path VARCHAR(1000), timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)"
cursor.execute(query)

con.commit()

# Default settings if not exist
cursor.execute("SELECT * FROM settings WHERE key='preferred_language'")
if cursor.fetchone() is None:
    cursor.execute("INSERT INTO settings VALUES (null, 'preferred_language', 'en')")
    con.commit()
    
cursor.execute("SELECT * FROM settings WHERE key='offline_mode'")
if cursor.fetchone() is None:
    cursor.execute("INSERT INTO settings VALUES (null, 'offline_mode', 'false')")
    con.commit()
