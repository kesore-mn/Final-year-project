$(document).ready(function () {
    ...
    eel.getSettings()(function(settings_data) {
        let data = JSON.parse(settings_data);
        console.log("Settings:", data);
        if (data) {
            data.forEach(setting => {
                if (setting[1] === 'GROQ_API_KEY') {
                   // Avoid showing key if not necessary, or show masked. 
                   // For now showing value.
                   $("#GroqApiKey").val(setting[2]);
                } else if (setting[1] === 'preferred_language') {
                    $("#LanguageSelect").val(setting[2]);
                } else if (setting[1] === 'offline_mode') {
                    $("#OfflineModeSwitch").prop('checked', setting[2] === 'true');
                }
            });
        }
    });
    ...
    $("#SaveGeneralBtn").click(function() {
        let apiKey = $("#GroqApiKey").val();
        let language = $("#LanguageSelect").val();
        let offlineMode = $("#OfflineModeSwitch").is(":checked") ? 'true' : 'false';

        eel.updateSetting('GROQ_API_KEY', apiKey);
        eel.updateSetting('preferred_language', language);
        eel.updateSetting('offline_mode', offlineMode);
